import type { Point, Product, FloorArea, NavigationNode, PathSegment, Language } from './types';
...
export function calculateMultiStopRoute(
    ...
    const remainingDestinations = destinations
        .map(d => {
            const area = areas.find(a => a.id === d.shopId);
            if (!area) {
                console.warn(`Pathfinding Warning: Could not find area for product with shopId: ${d.shopId}`);
                return null;
            }
            return {
                product: d,
